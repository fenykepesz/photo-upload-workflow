...
Take a screenshot. Confirm the photo loads and you can see it clearly. Zoom in if needed for better detail:
```
computer tool: zoom on the photo region
```
...
- **Keywords** — Generate **30–40 tags** for DeviantArt (DA allows up to 60). Use no-space or CamelCase for multi-word concepts — DA splits on spaces, so `long exposure` becomes two useless tags `long` and `exposure`; write it as `longexposure` instead.
...
Show the proposed metadata clearly before proceeding to the approval gate.
...
Get a browser tab via `tabs_context_mcp`, then `tabs_create_mcp` for a fresh tab.

**Navigate** to `https://500px.com` and take a screenshot.

**Account check:** Use `find` or `read_page` to locate the logged-in username/avatar. Compare it against `config.accounts["500px"]`. If not logged in or the wrong account is shown, stop and tell the user: *"500px is logged in as [name] but config expects [expected]. Please switch accounts, then let me know."* Wait for confirmation before continuing.

**Navigate** to `https://500px.com/photo/upload` and take a screenshot to confirm the upload page loaded.
...
Wait for upload progress to reach 100%. Take a screenshot.

**Fill metadata fields:**
1. Title → `{final_title}`
2. Description → `{final_caption}`
3. Tags → `{final_keywords}` (enter comma-separated or one by one)
4. Category → `{category_500px}` (select from dropdown)

**Publish:** Click Publish. Wait for confirmation. Take a screenshot.

Extract the resulting photo URL (e.g. `https://500px.com/photo/...`). Store as `url_500px`.

**On failure:** Take a screenshot, log the error, continue to the next platform. Mark 500px as failed in notes but do not stop the whole workflow.

---

### 6B. 35photo (if "35P" in platforms)

**Navigate** to `https://35photo.pro/en/` and take a screenshot.

**Account check:** Use `find` or `read_page` to locate the logged-in username. Compare against `config.accounts["35photo"]`. If not logged in or the wrong account is shown, stop and tell the user: *"35photo is logged in as [name] but config expects [expected]. Please switch accounts, then let me know."* Wait for confirmation before continuing.

**Navigate** to the upload section (look for Upload or Add Photo link in the nav).
...
**Fill metadata:**
1. Title → `{final_title}`
2. Description/Comment → `{final_caption}`
3. Tags → `{final_keywords}`
4. Category → `{category_35p}` (select from available genres)

**Submit.** Wait for confirmation. Take a screenshot.
...
Take a screenshot. Confirm you are logged in on the personal profile.

**Account check:** Use `find` or `read_page` to locate the logged-in profile name (top-right of the page). Compare against `config.accounts["facebook"]`. If not logged in or the wrong account is shown, stop and tell the user: *"Facebook is logged in as [name] but config expects [expected]. Please switch accounts, then let me know."* Wait for confirmation before continuing.

**Navigate** to the user's profile or create a post from the home feed.
...
**Fill the caption:**
- Use `caption_fb` field if it is not empty
- Otherwise use `caption` but strip all external social media URLs (Instagram, Twitter/X, DeviantArt, 500px links)
- Facebook will remove external links anyway — keep the caption clean

**Post.** Click the Post button. Wait for the post to appear. Take a screenshot.

Try to extract the post URL. Store as `url_fb` (may not always be available immediately).

**On failure:** Log the error. Note: Facebook is the most likely platform to request CAPTCHA or additional verification. If you see a CAPTCHA or security check, stop and tell the user: *"Facebook is showing a security check — please complete it manually, then let me know when to continue."*
...
Navigate to `https://www.deviantart.com` and take a screenshot. Look for the username in the top-right. Compare it against `config.accounts["deviantart"]`. If not logged in or the wrong account is shown, stop and tell the user: *"DeviantArt is logged in as [name] but config expects [expected]. Please switch accounts, then let me know."* Wait for confirmation before continuing.

**Navigate to the Sta.sh item:**

Go directly to the Sta.sh URL: `{stash_url_nsfw}`

Scroll below the photo until you see the "Edit or Submit" button (next to "Sell Deviation"). Click it.

**Fill the submission form:**

1. **Title** — Triple-click to select all, then type `{final_title}`
2. **Mature checkbox** — Check it only if `da_nsfw_flag == TRUE`. Leave unchecked if FALSE.
3. **Tags** — Remove all existing tags (click the × on each), then add each keyword from `{final_keywords}` one at a time, pressing Enter after each. Note: multi-word tags are split by spaces on DA — use CamelCase for compound tags (e.g. `streetphotography`, `SoutheastAsia`).
4. **Description** — Click the description field and type `{final_caption}`
5. **Gallery** — Leave as Featured (default) unless the user specifies otherwise

Take a screenshot of the filled form to verify it looks correct, then click **Submit**.

**After submission:** DeviantArt redirects to the live deviation page. The URL format is `https://www.deviantart.com/flyy1/art/{deviation_id}`. Capture this as `deviation_url`.
...
| DA browser: submission fails | Take a screenshot and report the error. Do not retry without user confirmation. |
